import random
import sys
import time
import numpy as np
from collections import deque
import argparse

# edge weight in the graph
W = 0.01
K = 200
snap_num = 500
graph_file = r"C:\Users\siwei\OneDrive\Paper\Final Code\SocialMediaComputing-master\sg\phy.txt"

# V: node Set
# S: Seed Set
V = set()
S = set()

# CELF conmponent
def R_v(adj, v, former_temp):
    temp = set()
    if not(v in former_temp):
        temp.add(v)
    s_temp = set({v})
    while s_temp:
        a = set()
        for i in s_temp:
            if not(i in adj):
                continue
            for j in adj[i]:
                if not(j in temp) and not(j in former_temp):
                    temp.add(j)
                    a.add(j)
        s_temp = a
    return len(temp)


# CELF conponent
def R_first_v(adj, v):
    temp = set({v})
    s_temp = set({v})
    while s_temp:
        a = set()
        for i in s_temp:
            if not(i in adj):
                continue
            for j in adj[i]:
                if not(j in temp):
                    temp.add(j)
                    a.add(j)
        s_temp = a
    return len(temp)

# Fun -- calculate the influence of the seed set
# adj -- the graph
# S -- seed set
def cal_R(adj, S):
    temp = set()
    s_temp = S.copy()
    temp = temp | s_temp
    while s_temp:
        a = set()
        for i in s_temp:
            if not(i in adj):
                continue
            for j in adj[i]:
                b = random.random()
                if b < W and not(j in temp):
                    temp.add(j)
                    a.add(j)
        s_temp = a

    return len(temp)

# CELF conmponent
def R_S(adj, v, former_temp):
    s_temp = set()
    if v == 1000000:
        return 0
    s_temp.add(v)
    former_temp.add(v)
    while s_temp:
        a = set()
        for i in s_temp:
            if not(i in adj):
                continue
            for j in adj[i]:
                if not(j in former_temp):
                    former_temp.add(j)
                    a.add(j)
        s_temp = a


if __name__ == '__main__':

    # take inputs, need further study later
    # not useful current
    parser = argparse.ArgumentParser()
    parser.add_argument('-k', type=int, default=None)
    parser.add_argument('-dat', type=str, default=None)
    args = parser.parse_args()

    # starting point of the algorithm
    t_start = time.time()
    f = open(graph_file,"r")
    adj={}
    G = []
    cnt = 0
    # 100 graphs
    for i in range(snap_num):
        G.append({})
    for line in f.readlines()[4:]:
        a = [int(s) for s in line.replace('\n', '').split(' ')]
        b = int(a[0])
        c = int(a[1])

        if not(b in adj):
            adj[b] = []
        adj[b].append(c)

        for j in range(snap_num):
            if random.random() < W:
                if not(b in G[j]):
                   G[j][b] = []
                G[j][b].append(c)
    t_adj = time.time()
    print("generating adj time: ", t_adj - t_start)
    V = set(adj.keys())
    print("Finishing generating Gi!")

    influence_dict = {}

    for i in V:
        num = 0
        for j in G:
            # for every snapshot
            num += R_first_v(j, i)
        influence_dict[i] = num
    print("cal_time: ", time.time() - t_adj)
    order = sorted(influence_dict, key = lambda a: influence_dict[a], reverse=True)
    print(order)
    former_temp = []
    for i in range(len(G)):
        former_temp.append(set())
    S.add(order[0])
    print("step: ", 0, "num", influence_dict[order[0]], "S", S)
    for j in range(len(G)):
        R_S(G[j], order[0], former_temp[j])

    influence_dict.pop(order[0])
    order.remove(order[0])
    for i in range(K-1):
        k = 0
        index_ = 0
        maximum = 0
        while 1:
            v = order[k]
            num = 0
            for j in range(len(G)):
                num += R_v(G[j], v, former_temp[j])

            influence_dict[v] = num
            if num > maximum:
                maximum = num
                index = k
            influence_dict[v] = num
            if influence_dict[order[k]] < influence_dict[order[k + 1]]:
                k += 1
            else:
                break

            if maximum > influence_dict[order[k + 1]]:
                k = index
                break
        S.add(order[k])
        for j in range(len(G)):
            R_S(G[j], order[k], former_temp[j])

        print("step: ", i + 1, "num", influence_dict[order[k]], "S", S)
        influence_dict.pop(order[k])
        order = sorted(influence_dict, key = lambda a: influence_dict[a], reverse=True)

    t_finds = time.time()
    t = t_finds - t_start
    print("Time: ", t)
    print(S)

    # calculate the influence of seed set in the graph
    # S: Seed set
    # adj: the graph
    cnt = 0
    for i in range(10000):
        cnt += cal_R(adj, S)
    cnt /= 10000
    #output Result
    print("influence: ", cnt)
    print("cal_time: ", time.time() - t_finds)
