...
class Env:
    ...
    budget = 500,
        ...
        self.list = strength_list(self.graph,ratio)
...
def strength_list(g,ratio):
    A = {};
    V = set(g.keys())
    max_increment = {i:IC(g, {i}) for i in g.keys()}
    t = sorted(max_increment.items(), key=lambda x:x[1], reverse=True)
    index = 0;
    k = math.floor(len(g)*ratio)
    A = t[0:k]
    return A;


#test = Env(r"C:\Users\siwei\Desktop\Paper\Final Code\DATA\hep.txt")
#print(test.maxGain)
