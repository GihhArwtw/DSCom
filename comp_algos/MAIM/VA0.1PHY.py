...
# ------------changed file path----------------
Data = r"/content/drive/MyDrive/data/NetPHY.txt"
...
budget = 70 # size of seed node
Greedy = 0.5 # P to apply policy
Memory_size = budget*2 # memory size but why?
...
class DeepQNetwork:
    ...
    def choose_action(self, feat_cur):
        ...
        if np.random.uniform() < self.epsilon:
            ...
            reject = self.sess.run(self.q_eval, feed_dict={self.s: observation}) # q value replacement
    ...
    def reject_dis(self): # accept - reject
...
def run_it():
    ...
    # test times
        ...
        while(True):
            ...
            print("length of seed:",len(test.seed))
            ...
            if action == 0: #accepted\
                ...
                input_at_step_test = test.netInput # original state
            ...
            RL.store_transition(input_at_step_test, action, reward, test.netInput) # (s,a,r,s_)
