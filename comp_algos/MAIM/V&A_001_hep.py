...
Data = r"C:\Users\siwei\OneDrive\Paper\DATA\Cit-HepPh.txt"
testData =r"C:\Users\siwei\OneDrive\Paper\DATA\Cit-HepPh.txt"
    ...
    # test times
        ...
        while(True):
            ...
            if len(test.seed)%10 == 0  and len(test.seed)!= 0:
                print(len(test.seed),' ',EN.IC(test.graph,test.seed),' ',Time,' ',flush=True)
        ...
        print("Other influence is: ", EN.IC(test.graph,{211331, 207108, 301061, 209287, 202122, 204299, 212237, 210192, 210323, 212123, 204316, 212380, 106012, 5025, 208043, 9712301, 105261, 211252, 212406, 112312, 201146, 201156, 202058, 210251, 108236, 211404, 302155, 302031, 212304, 208209, 203219, 210390, 101336, 206169, 3033, 204251, 209244, 203229, 111198, 301023, 211168, 302174, 302054, 101224, 207213, 201071, 210291, 206072, 206077, 204031}),flush=True )
        process = psutil.Process(os.getpid())
        end_M = process.memory_info().rss  # in bytes
        print("Memory used",end_M - start_M, "bytes", flush=True)  # in bytes
        input()
